...
if [[ $install_aliases == [Yy] ]]; then
    ...
    echo "$jellyfin_aliases" >> ~/.bashrc

    # Source bashrc
    source ~/.bashrc
    echo "Alises installed. You can now manage the Jellyfin service using the following commands:"
    echo "restart_jellyfin, start_jellyfin, stop_jellyfin"
fi
